...
/**
 * Controller class for the View Reports screen.
 * This screen allows the Manager to view two types of monthly reports:
 * 1. Performance Report (Bar Chart) - Shows delays, arrival times, and customer lateness.
 * 2. Orders Activity Report (Line Chart) - Shows the number of orders and waitlist entries per day.
 * * The data is fetched from the server based on the selected month and year.
 */
public class ViewReportsController implements Initializable {
    ...
    @FXML private Button btnLoadReport;
    @FXML private Label lblStatus;
    @FXML private Button btnClose;

    // Charts for Performance Report
    @FXML private BarChart<String, Number> barChartPerformance;
    @FXML private CategoryAxis xAxisPerf;
    @FXML private NumberAxis yAxisPerf;

    // Charts for Activity Report
    @FXML private LineChart<String, Number> lineChartSubscriptions;
    @FXML private CategoryAxis xAxisSub;
    @FXML private NumberAxis yAxisSub;

    /**
     * Called when the screen is initialized.
     * Sets up the report types in the combo box and sets the default date to the current month.
     */
    @Override
    public void initialize(URL location, ResourceBundle resources) {
        ...
        // Fill days and years
        for (int i = 1; i <= 12; i++) cmbMonth.getItems().add(i);
    ...
    /**
     * Main action when the user clicks 'Generate'.
     * Checks if the date is valid and calls the relevant method to load the data.
    ...
    /**
     * Handles the logic for the Performance Report.
     * Sends a request to the server and updates the Bar Chart with the data received.
     * It displays metrics like Average Delay, Lateness, and Stay Duration.
     * * @param date The month and year array.
     */
    ...
    /**
     * Handles the logic for the Orders Activity Report.
     * Updates the Line Chart with two lines: one for Orders and one for Waitlist entries.
     * It iterates through all days of the month (1-31) to build the graph.
     * * @param date The month and year array.
     */
